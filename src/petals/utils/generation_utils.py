...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
        # for now we simply set max length here
        model_kwargs['max_length'] = 20

        # forward pass to get next token
        outputs, timestamp = self(
            **model_inputs,
            return_dict=True,
            output_attentions=output_attentions,
        ...
        iteration_info = model_kwargs.get('iteration_info')
        ...
        req_input_ids_list = []
        length_idx = 0
        for length in input_length_table:
            req_input_ids = input_ids[:, length_idx:length_idx + length]
            req_input_ids_list.append(req_input_ids)
            length_idx += length
        
        req_next_token_logits_list = []
        #req_outputs_list = []
        token_num_idx = 0
        idx = 0
        for token_num in iter_token_num_table:
            #req_outputs = outputs[:, token_num_idx:token_num_idx+token_num, :]
            #req_next_token_logits = next_token_logits[:, token_num_idx:token_num_idx+token_num, :]
            req_next_token_logits = outputs.logits[:, token_num_idx+token_num-1, :]
            #req_outputs_list.append(req_outputs)
            req_next_token_logits_list.append(req_next_token_logits)
            token_num_idx += token_num
            idx += 1

        
        next_tokens_list = []
        for idx in range(batch_size):
            #req_outputs = req_outputs_list[idx]
            req_input_ids = req_input_ids_list[idx]
            req_next_token_logits = req_next_token_logits_list[idx]

            # pre-process distribution
            req_next_token_scores = logits_processor(req_input_ids, req_next_token_logits)
            if do_sample:
                req_next_token_scores = logits_warper(req_input_ids, req_next_token_scores)
            
            # token selection
            if do_sample:
                probs = nn.functional.softmax(req_next_token_scores, dim=-1)
                req_next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
            else:
                req_next_tokens = torch.argmax(req_next_token_scores, dim=-1)

            # finished sentences should have their next token be a padding token
            if has_eos_stopping_criteria:
                req_next_tokens = req_next_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)
            
            next_tokens_list.append(req_next_tokens)

        # the following processing are certainly incorrect
        # but it seems that we don't have to handle this in Petals
        # Store scores, attentions and hidden_states when required
        '''
        if return_dict_in_generate:
            if output_scores:
            ...
            if output_attentions:
                decoder_attentions += (
                    (req_outputs.decoder_attentions,) if self.config.is_encoder_decoder else (req_outputs.attentions,)
                )
                if self.config.is_encoder_decoder:
                    cross_attentions += (outputs.cross_attentions,)
        ...
        next_tokens = torch.concat(next_tokens_list, dim=-1)
        iteration_info = model_kwargs.get('iteration_info')
        input_length_table = iteration_info['input_length_table']
        iter_token_num_table = iteration_info['iter_token_num_table']
        new_input_ids_list = []
        ...
        assert len(input_length_table) == next_tokens.shape[0]

        for length in input_length_table:
            #req_input_ids = input_ids[:, length_idx:length, :]
            req_next_tokens = next_tokens[idx].unsqueeze(0)
            req_input_ids = torch.concat((input_ids[:, length_idx:length_idx + length], req_next_tokens[:, None]), dim = -1)
            new_input_ids_list.append(req_input_ids)
            idx += 1
            length_idx += length
        
        input_ids = torch.concat((new_input_ids_list), dim=-1)

        model_kwargs['iteration_info']['prefix_length_table'] = input_length_table
        input_length_table = [length + 1 for length in input_length_table]
        iter_token_num_table = [1 for num in iter_token_num_table]
        model_kwargs['iteration_info']['input_length_table'] = input_length_table
        model_kwargs['iteration_info']['iter_token_num_table'] = iter_token_num_table
        model_kwargs['iteration_info']['stage'] = 'decode'

        # get eos token id
        eos_token_id = generation_config.eos_token_id
        max_new_tokens_table =  model_kwargs['iteration_info']['max_new_tokens_table']

        finished_table = [False for i in range(batch_size)]
        for i in range(next_tokens.shape[0]):
            if next_tokens[i] == eos_token_id:
                finished_table[i] = True
        
        for i in range(batch_size):
            if input_length_table[i] >= max_new_tokens_table[i]:
                finished_table[i] = True

        info = (input_length_table, finished_table)

        te = time.time()
        print('process time inside _sample: {}'.format(te - ts))

        te = time.time()
        print('post processing time: {}'.format(te - timestamp))
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                dict_result = GenerateEncoderDecoderOutput(
                    sequences=input_ids,
                    scores=scores,
                    logits=raw_logits,
                    encoder_attentions=encoder_attentions,
                ...
                return (dict_result, info)
            else:
                dict_result = GenerateDecoderOnlyOutput(
                    sequences=input_ids,
                    scores=scores,
                    logits=raw_logits,
                    attentions=decoder_attentions,
