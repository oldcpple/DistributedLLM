# we rewrite original tensor_descr.py in Petals
# in order to support managing attention cache
# at the granularity of each request (corresponding to a single prompt)
...
@dataclass(init=True, repr=True, frozen=True)
class NewTensorDescriptor(DescriptorBase):
    request_id: id
    size: tuple
    ...
    def make_zeros(self, **kwargs):
        ...
        properties['requires_grad'] = self.requires_grad
        properties['pin_memory'] = self.pin_memory
...
class BatchTensorDescriptor(NewTensorDescriptor):
    ...
    def __init__(self, *instance_size, **kwargs):  # compatibility: allow initializing with *size
        ...
        super().__init__(None, (None, *instance_size), **kwargs)
